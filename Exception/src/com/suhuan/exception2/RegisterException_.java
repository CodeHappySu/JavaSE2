 ...
 */
public class RegisterException_ extends Exception {//让这个类变成一个运行时异常类


    public RegisterException_() {
    }

    public RegisterException_(String message) {
        super(message);
    }
}
class Demo2{
    ...
    public static void checkName(String name){
        for (int i = 0; i < arr.length; i++) {
            if(arr[i].equals(name)){//说明用户存在
                try {
                    throw new RegisterException_("用户名"+name+"已经被占用！");
                } catch (RegisterException_ registerException_) {
                    registerException_.printStackTrace();
                    return;//手动结束方法，不然后面的语句继续执行
                }
            }
        }
